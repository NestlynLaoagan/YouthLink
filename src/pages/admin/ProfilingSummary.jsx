...
export default function ProfilingSummary() {
  ...
  // Verification removed — profiles are auto-verified on submission

  const exportSpreadsheet = () => {
    const cols = ['Name','Email','Contact Number','Purok','Work Status','Civil Status','Birthday','Age','Gender']
    const rows = filtered.map(p => [p.name||'',p.email||'',p.contact_number||'N/A',p.address?.split(',')?.[0]?.replace('Purok','').trim()||'N/A',p.work_status||'N/A',p.civil_status||'N/A',p.birthday||'',p.age||'',p.gender||''])
  ...
  const verifiedProfiles = profiles  // All profiles included — verification removed
  const chartData = {
