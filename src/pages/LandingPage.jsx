...
function AnnCard({ a, idx, dark=true, onClick }) {
  const { icon, color } = TYPE_META[a.type] || { icon:'📌', color:'#FBBF24' }
  const sc = STATUS_COLOR[(a.status||'').toLowerCase()] || '#64748B'
  const cardStyle = dark
    ? { padding:'14px 16px', cursor:'pointer', animation:`slideL .5s ease ${idx*0.1}s both`, transition:'transform .15s, box-shadow .15s' }
    : { padding:'14px 16px', cursor:'pointer', animation:`slideL .5s ease ${idx*0.1}s both`,
        background:'rgba(255,255,255,0.95)', border:'1px solid rgba(203,213,225,0.7)',
        boxShadow:'0 2px 10px rgba(0,0,0,0.05)', borderRadius:16, transition:'transform .15s, box-shadow .15s' }
  return (
    <div className={dark ? 'glass' : ''} style={cardStyle} onClick={onClick}
      onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow=dark?'0 8px 24px rgba(0,0,0,0.5)':'0 6px 20px rgba(0,0,0,0.1)'}}
...
/* ── Full-Detail Modal (Announcements, Projects, Events) ── */
function DetailModal({ item, type, dark, onClose }) {
  useEffect(() => {
  ...
  const overlay = { position:'fixed', inset:0, zIndex:1000,
    background:'rgba(0,0,0,0.72)', backdropFilter:'blur(6px)',
    display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }
  const modal = {
    background: dark ? 'rgba(8,16,40,0.97)' : '#ffffff',
    border: dark ? '1px solid rgba(96,165,250,0.2)' : '1px solid rgba(203,213,225,0.9)',
    borderRadius:18, padding:'0', width:'100%', maxWidth:520,
    maxHeight:'88vh', overflowY:'auto', position:'relative',
    boxShadow: dark ? '0 24px 60px rgba(0,0,0,0.7), 0 0 40px rgba(96,165,250,0.08)' : '0 24px 60px rgba(0,0,0,0.15)',
    animation:'calPop .25s ease both'
  }

  const STATUS_C = { upcoming:'#60A5FA', ongoing:'#34D399', planning:'#A78BFA', completed:'#34D399', cancelled:'#F87171', finished:'#64748B' }
  const TYPE_META = { 'Sports':{ icon:'🏃', color:'#F87171' }, 'Assembly':{ icon:'📢', color:'#60A5FA' }, 'General':{ icon:'📋', color:'#FBBF24' }, 'Seminar':{ icon:'🎓', color:'#A78BFA' }, 'Training & Workshop':{ icon:'🔧', color:'#34D399' } }

  const renderContent = () => {
    if (type === 'announcement') {
      const { icon, color } = TYPE_META[item.type] || { icon:'📌', color:'#FBBF24' }
      const sc = STATUS_C[(item.status||'').toLowerCase()] || '#64748B'
      return (
        <>
          {/* Header strip */}
          <div style={{ background:`linear-gradient(135deg,${color}22,${color}08)`, borderBottom: dark ? `1px solid ${color}25` : `1px solid ${color}30`, padding:'20px 22px 16px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10 }}>
              <div style={{ width:44, height:44, borderRadius:12, background:`${color}1a`, border:`1.5px solid ${color}35`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, flexShrink:0 }}>{icon}</div>
              <div style={{ flex:1, minWidth:0 }}>
                <span style={{ fontSize:9, fontWeight:800, padding:'2px 10px', borderRadius:20, background:`${sc}18`, color:sc, border:`1px solid ${sc}35`, textTransform:'capitalize', fontFamily:M }}>{item.status||'upcoming'}</span>
                {item.type && <span style={{ fontSize:9, fontWeight:700, padding:'2px 10px', borderRadius:20, background:`${color}15`, color, border:`1px solid ${color}30`, fontFamily:F, marginLeft:6 }}>{item.type}</span>}
              </div>
            </div>
            <p style={{ fontSize:16, fontWeight:700, color: dark?'#E2E8F0':'#0f172a', margin:0, fontFamily:F, lineHeight:1.35 }}>{item.title}</p>
          </div>
          {/* Body */}
          <div style={{ padding:'18px 22px 22px' }}>
            {(item.date_time || item.created_at) && (
              <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:14, padding:'8px 12px', borderRadius:9, background: dark?'rgba(96,165,250,0.06)':'rgba(37,99,235,0.05)', border: dark?'1px solid rgba(96,165,250,0.12)':'1px solid rgba(37,99,235,0.1)' }}>
                <span style={{ fontSize:13 }}>📅</span>
                <span style={{ fontSize:12, color: dark?'#93C5FD':'#2563eb', fontWeight:600, fontFamily:F }}>
                  {new Date(item.date_time||item.created_at).toLocaleDateString('en-PH',{ weekday:'long', month:'long', day:'numeric', year:'numeric' })}
                  {item.date_time && ' · ' + new Date(item.date_time).toLocaleTimeString('en-PH',{ hour:'2-digit', minute:'2-digit' })}
                </span>
              </div>
            )}
            <p style={{ fontSize:13, color: dark?'#CBD5E0':'#334155', lineHeight:1.75, margin:0, fontFamily:F, whiteSpace:'pre-wrap' }}>{item.content}</p>
            {item.location && (
              <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:14, fontSize:12, color: dark?'#94A3B8':'#64748b', fontFamily:F }}>
    ...
    if (type === 'project') {
      const STATUS_C2 = { upcoming:'#60A5FA', ongoing:'#34D399', planning:'#A78BFA', completed:'#34D399' }
      const color = STATUS_C2[(item.status||'').toLowerCase()] || '#60A5FA'
      const imgs = (item.images||[]).filter(Boolean)
      return (
        <>
          {/* Image */}
          {imgs.length > 0 && (
            <div style={{ position:'relative', width:'100%', paddingBottom:'45%', background:'#040d1e', overflow:'hidden', borderRadius:'18px 18px 0 0' }}>
              <img src={imgs[0]} alt={item.project_name} style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }}/>
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(0deg,rgba(2,8,23,.65) 0%,transparent 55%)' }}/>
            </div>
          )}
          {/* Header */}
          <div style={{ padding:'18px 22px 14px', borderBottom: dark?'1px solid rgba(96,165,250,0.1)':'1px solid rgba(203,213,225,0.6)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10, flexWrap:'wrap' }}>
              <span style={{ fontSize:9, fontWeight:800, padding:'3px 12px', borderRadius:20, background:`${color}18`, color, border:`1px solid ${color}30`, textTransform:'capitalize', fontFamily:M }}>{item.status||'upcoming'}</span>
              {item.budget && <span style={{ fontSize:12, fontWeight:700, color:'#FBBF24', fontFamily:F }}>₱{parseFloat(item.budget).toLocaleString()}</span>}
              {item.fund_source && <span style={{ fontSize:9, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'rgba(251,191,36,0.12)', color:'#FBBF24', border:'1px solid rgba(251,191,36,0.25)', fontFamily:F }}>{item.fund_source}</span>}
            </div>
            <p style={{ fontSize:17, fontWeight:700, color: dark?'#E2E8F0':'#0f172a', margin:0, fontFamily:M, lineHeight:1.3 }}>{item.project_name}</p>
          </div>
          {/* Body */}
          <div style={{ padding:'16px 22px 22px' }}>
            {item.description && <p style={{ fontSize:13, color: dark?'#CBD5E0':'#334155', lineHeight:1.75, margin:'0 0 16px', fontFamily:F, whiteSpace:'pre-wrap' }}>{item.description}</p>}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {item.prepared_by && <div style={{ padding:'8px 12px', borderRadius:9, background: dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)', border: dark?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(0,0,0,0.07)' }}><p style={{ fontSize:9, color: dark?'#64748B':'#94a3b8', fontFamily:F, margin:'0 0 3px', textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>Prepared by</p><p style={{ fontSize:12, color: dark?'#E2E8F0':'#0f172a', fontFamily:F, margin:0, fontWeight:600 }}>{item.prepared_by}</p></div>}
              {item.start_date && <div style={{ padding:'8px 12px', borderRadius:9, background: dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)', border: dark?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(0,0,0,0.07)' }}><p style={{ fontSize:9, color: dark?'#64748B':'#94a3b8', fontFamily:F, margin:'0 0 3px', textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>Start Date</p><p style={{ fontSize:12, color: dark?'#E2E8F0':'#0f172a', fontFamily:F, margin:0, fontWeight:600 }}>{new Date(item.start_date).toLocaleDateString('en-PH',{month:'long',day:'numeric',year:'numeric'})}</p></div>}
              {item.end_date && <div style={{ padding:'8px 12px', borderRadius:9, background: dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)', border: dark?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(0,0,0,0.07)' }}><p style={{ fontSize:9, color: dark?'#64748B':'#94a3b8', fontFamily:F, margin:'0 0 3px', textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>End Date</p><p style={{ fontSize:12, color: dark?'#E2E8F0':'#0f172a', fontFamily:F, margin:0, fontWeight:600 }}>{new Date(item.end_date).toLocaleDateString('en-PH',{month:'long',day:'numeric',year:'numeric'})}</p></div>}
              {item.location && <div style={{ padding:'8px 12px', borderRadius:9, background: dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)', border: dark?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(0,0,0,0.07)' }}><p style={{ fontSize:9, color: dark?'#64748B':'#94a3b8', fontFamily:F, margin:'0 0 3px', textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>Location</p><p style={{ fontSize:12, color: dark?'#E2E8F0':'#0f172a', fontFamily:F, margin:0, fontWeight:600 }}>{item.location}</p></div>}
            </div>
            {imgs.length > 1 && (
              <div style={{ marginTop:14 }}>
                <p style={{ fontSize:9, color: dark?'#64748B':'#94a3b8', fontFamily:F, margin:'0 0 8px', textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>Photos</p>
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {imgs.slice(1).map((img,i)=><img key={i} src={img} alt="" style={{ width:72, height:72, borderRadius:8, objectFit:'cover', border: dark?'1px solid rgba(255,255,255,0.08)':'1px solid rgba(0,0,0,0.08)' }}/>)}
                </div>
              </div>
            )}
          </div>
    ...
    if (type === 'event') {
      ...
      return (
        <>
          <div style={{ background:`linear-gradient(135deg,${sc}18,${sc}06)`, borderBottom: dark?`1px solid ${sc}25`:`1px solid ${sc}30`, padding:'20px 22px 16px' }}>
          ...
          <div style={{ padding:'18px 22px 22px' }}>
            {item.start_date && (
              <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10, padding:'8px 12px', borderRadius:9, background: dark?'rgba(96,165,250,0.06)':'rgba(37,99,235,0.05)', border: dark?'1px solid rgba(96,165,250,0.12)':'1px solid rgba(37,99,235,0.1)' }}>
                <span style={{ fontSize:13 }}>📅</span>
                <span style={{ fontSize:12, color: dark?'#93C5FD':'#2563eb', fontWeight:600, fontFamily:F }}>
                  {new Date(item.start_date).toLocaleDateString('en-PH',{ weekday:'long', month:'long', day:'numeric', year:'numeric' })}
                </span>
              </div>
            )}
            {item.end_date && item.end_date !== item.start_date && (
              <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10, padding:'8px 12px', borderRadius:9, background: dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)', border: dark?'1px solid rgba(255,255,255,0.07)':'1px solid rgba(0,0,0,0.08)' }}>
                <span style={{ fontSize:13 }}>🏁</span>
                <span style={{ fontSize:12, color: dark?'#94A3B8':'#475569', fontWeight:600, fontFamily:F }}>Until {new Date(item.end_date).toLocaleDateString('en-PH',{ weekday:'long', month:'long', day:'numeric', year:'numeric' })}</span>
              </div>
            )}
            {item.description && <p style={{ fontSize:13, color: dark?'#CBD5E0':'#334155', lineHeight:1.75, margin:'8px 0 0', fontFamily:F, whiteSpace:'pre-wrap' }}>{item.description}</p>}
            {item.location && (
              <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:14, fontSize:12, color: dark?'#94A3B8':'#64748b', fontFamily:F }}>
                <span>📍</span><span>{item.location}</span>
              </div>
    ...
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={e=>e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{
          ...
          color: dark?'#94A3B8':'#64748b', fontSize:16, fontWeight:700,
          display:'flex', alignItems:'center', justifyContent:'center',
          transition:'all .15s'
        }}
          onMouseEnter={e=>{e.currentTarget.style.background='rgba(248,113,113,0.2)';e.currentTarget.style.color='#F87171'}}
          onMouseLeave={e=>{e.currentTarget.style.background=dark?'rgba(255,255,255,0.1)':'rgba(0,0,0,0.08)';e.currentTarget.style.color=dark?'#94A3B8':'#64748b'}}>
          ✕
        </button>
        {renderContent()}
      </div>
...
function ProjCarousel({ projects, label, color, goLogin, dark=true, onCardClick }) {
    ...
    <div onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}
      ...
      <div key={`proj-${cur}`}
        ...
        style={{ background: dark ? 'rgba(15,23,42,0.75)' : '#ffffff', borderRadius:14,
          border: dark ? `1px solid ${color}25` : '1px solid rgba(203,213,225,0.8)', overflow:'hidden', cursor:'pointer',
...
function Calendar({ events, dark=true, onEventClick }) {
  ...
  const evOnDay = d => events.filter(ev => {
    try {
      ...
      const start = new Date(ev.start_date || ev.created_at)
      start.setHours(0,0,0,0)
      // If event has an end_date, highlight every day in the range
      if (ev.end_date) {
        const end = new Date(ev.end_date)
        end.setHours(23,59,59,999)
        return target >= start && target <= end
      }
      // Single-day event — match start date only
  ...
  const handleDayClick = (d, e) => {
    ...
    if (evs.length === 1 && onEventClick) {
      // Single event — open detail modal directly
      onEventClick(evs[0])
    } else {
      // Multiple events — show popup list first
          ...
          <div style={{
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {tooltip.evs.map((ev,i)=>{
                ...
                return (
                  <div key={i}
                    onClick={e=>{ e.stopPropagation(); if(onEventClick){ setTooltip(null); onEventClick(ev) } }}
                    style={{
                    ...
                    onMouseEnter={e=>{e.currentTarget.style.background=dark?'rgba(96,165,250,0.12)':'rgba(37,99,235,0.07)';e.currentTarget.style.transform='translateX(2px)'}}
                    onMouseLeave={e=>{e.currentTarget.style.background=dark?'rgba(255,255,255,0.03)':'rgba(0,0,0,0.02)';e.currentTarget.style.transform='translateX(0)'}}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize:12, fontWeight:600, color: dark ? '#E2E8F0' : '#0f172a', margin:'0 0 2px',
                        fontFamily:'Sora,sans-serif', lineHeight:1.3,
                        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {ev.title}
                      </p>
                      <p style={{ fontSize:9, color: dark?'#60A5FA':'#2563eb', margin:0, fontFamily:'Sora,sans-serif', fontWeight:600 }}>Click to view full details →</p>
                    </div>
...
export default function LandingPage() {
  ...
  const goSignup = () => navigate('/login', { state: { tab: 'signup' } })
  ...
  return (
    <div style={{ minHeight:'100vh', fontFamily:F, position:'relative', overflowX:'hidden', background:bg, transition:'background .3s' }}>
        ...
        <div style={{ maxWidth:1160, margin:'0 auto', padding:'0 20px' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:20, animation:'fadeUp .6s ease both' }}>
            ...
            <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginBottom:26 }}>
              <button className="cta-red" onClick={goSignup}
          ...
          <div style={{ display:'grid',
            ...
            <div id="announcements">
              ...
              {anns.length === 0 ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {anns.slice(0,3).map((a,i) => <AnnCard key={a.id} a={a} idx={i} dark={dark} onClick={()=>openModal(a,'announcement')}/>)}
                </div>
              )}
              <button onClick={goLogin}
            ...
            <div id="events">
              ...
              <Calendar events={events.filter(ev => (ev.status||'').toLowerCase() !== 'cancelled')} dark={dark} onEventClick={ev=>openModal(ev,'event')}/>
            </div>
              ...
              {(() => {
                ...
                return upcoming.length > 0 ? (
                  <div>
                    ...
                    <ProjCarousel projects={upcoming} label="Upcoming" color={dark?"#60A5FA":"#2563eb"} goLogin={goLogin} dark={dark} onCardClick={p=>openModal(p,'project')}/>
                    <button onClick={goLogin}
              ...
              {(() => {
                ...
                return accomplished.length > 0 ? (
                  <div>
                    ...
                    <ProjCarousel projects={accomplished} label="Accomplished" color={dark?"#34D399":"#059669"} goLogin={goLogin} dark={dark} onCardClick={p=>openModal(p,'project')}/>
                    <button onClick={goLogin}
                      style={{ display:'flex', alignItems:'center', gap:4, background:'none', border:'none',
      ...
      {/* ── Full-Detail Modal ── */}
      {modal && <DetailModal item={modal.item} type={modal.type} dark={dark} onClose={closeModal}/>}
    </div>
  )
}
