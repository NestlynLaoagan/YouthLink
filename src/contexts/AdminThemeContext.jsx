...
export function AdminThemeProvider({ children }) {
  ...
  const setPalette = (p) => { setPaletteState(p); localStorage.setItem('admin_palette', p) }
  const toggleDark = (v) => { setDark(v); localStorage.setItem('admin_dark', v) }
  const toggleNavbar = (v) => { setNavbarVisible(v); localStorage.setItem('admin_navbar', v) }
  const saveCustomColors = (colors) => { setCustomColors(colors); localStorage.setItem('admin_custom_colors', JSON.stringify(colors)) }
